...
bool equi_stratum_notify(struct stratum_ctx* sctx, json_t* params)
{
	...
	if (ntime > sctx->srvtime_diff) {
		...
		if (opt_protocol && ntime > 20)
			...
			printf("stratum time is at least %ds in the future", ntime);
	}
...
bool equi_stratum_show_message(struct stratum_ctx* sctx, json_t* id, json_t* params)
{
	...
	if (val) {
		...
		if (data && strlen(data)) {
			char symbol[32] = { 0 };
			uint32_t height = 0;
//			int ss = sscanf_s(data, "equihash %s block %u", symbol, &height);
...
bool equi_stratum_submit(struct pool_infos* pool, struct work* work)
{
	...
	if (!solhex || !noncestr) {
		...
		printf("unable to alloc share memory");
		return false;
	}
...
//	_snprintf_s(s, sizeof(s), "{\"method\":\"mining.submit\",\"params\":"
//		"[\"%s\",\"%s\",\"%s\",\"%s\",\"%s\"], \"id\":%u}",
...
//		stratum.job.shares_count + 10);
	...
	if (!stratum_send_line(&stratum, s)) {
		...
		printf("%s stratum_send_line failed", __func__);
		return false;
	}
