...
#include <stdlib.h>
#include <string.h>
//#include <unistd.h> // Thay bằng getopt
#include <getopt.h>
#include <assert.h>
...
extern "C" int scanhash_verus(int thr_id, struct work *work, uint32_t start_nonce, uint32_t max_nonce, unsigned long *hashes_done)
{
	...
	uint32_t nonce_buf = start_nonce;
	uint32_t fixrand[32];
...
//		memcpy(nonceSpace + 4, &pdata[EQNONCE_OFFSET + 1], 3 );		// the 15 bytes available
	...
	while (true) {
		if (stop_threads.load()) {
			printf("Stop thread %d by stop_thread\n", thr_id);
            LOGI("Stop thread %d by stop_thread\n", thr_id);
			break;
		}

//		if (mined.load()) {
//            Htarg = 100000;
//            nonce_buf = mined_nonce_buf.load();
//			throughput = mined_nonce_throughput.load();
//			printf("Stop thread %d by mined\n", thr_id);
//            LOGI("Stop thread %d by mined, buf = %d", thr_id, nonce_buf);
////			break;
//		}
		...
		*hashes_done = nonce_buf + throughput;
		...
		Verus2hash((unsigned char *)vhash, (unsigned char *)blockhash_half, (unsigned char*)nonceSpace, data_key, 
				&gpuinit, fixrand, fixrandex , data_key_prand, data_key_prandex, version);

		if (nonce_buf % 10000000 == 0) {
			printf("========> %d \n", Htarg);
			printf("%d\n", nonce_buf);
            LOGI("thread %d at nonce_buf %u", thr_id, nonce_buf);
		}

		if (vhash[7] <= Htarg)
		{
//			mined.store(true);
//            mined_nonce_buf.store(nonce_buf);
//			mined_nonce_throughput.store(throughput);

			printf("Founded nonce_buf = %d\n", nonce_buf);
			LOGI("Founded nonce_buf = %d\n", nonce_buf);
			work->valid_nonces++;
...
extern "C" int scanhash_verus_count(int thr_id, struct work *work, uint32_t start_nonce, uint32_t max_nonce, unsigned long *hashes_done)
{
    uint32_t *pdata = work->data;
    uint32_t *ptarget = work->target;
    uint8_t blockhash_half[64] = { 0 };
